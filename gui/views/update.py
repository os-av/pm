...
class UpdateView(Gtk.Grid):
    ...
    def set_fields(self):
        self.account.set_text(self.credential[1])
        self.username.set_text(self.credential[2])
        self.password.set_text(self.credential[3])
        self.url.set_text(self.credential[4])
        self.email.set_text(self.credential[5])
        notes_buffer = self.notes.get_buffer()
        notes_buffer.set_text(self.credential[6])
        
    def on_confirm_clicked(self, button):
        new_data = [
            None,
            self.account.get_text(),
            self.username.get_text(),
            self.password.get_text(),
            self.url.get_text(),
            self.email.get_text(),
            self.notes.get_buffer().get_text(
                self.notes.get_buffer().get_start_iter(),
                self.notes.get_buffer().get_end_iter(),
                True)]
        db = [None, "account", "username", "password", 
              "website", "email", "notes"]

        if len(new_data[1]) == 0 or len(new_data[2]) == 0 or len(new_data[3]) == 0:
            self.error.set_text("A required field is empty.")
            return

        if new_data[1] != self.credential[1]:
            if self.window.pm.read_credential_by_account(new_data[1]):
                self.error.set_text("An account using this name already exists.")
                return

        for i in range(1, len(new_data)):
            if new_data[i] != self.credential[i]:
                self.window.pm.update_credential_by_id(db[i], 
                                                self.credential[0],
                                                new_data[i])
